...
struct PrepareProof {
  ...
  PrepareProof(uint32_t v, uint64_t n, uint256_t d) : view(v), seq_num(n), digest(d) {}
...
struct ViewChangeMsg {
  ...
  ViewChangeMsg(uint32_t nv, uint64_t lsc, uint32_t rid, 
                const std::vector<CheckpointMsg>& C, const std::vector<PrepareProof>& P)
  : next_view(nv), last_stable_checkpoint(lsc), replica_id(rid), checkpoint_proof(C), prepared_proofs(P) {
    serialized << next_view << last_stable_checkpoint << replica_id << checkpoint_proof << prepared_proofs;
  }
  ViewChangeMsg(DataStream &&s) { 
    s >> next_view >> last_stable_checkpoint >> replica_id >> checkpoint_proof >> prepared_proofs; 
  }
...
struct NewViewMsg {
  ...
  NewViewMsg(uint32_t nv, const std::vector<ViewChangeMsg> &V, const std::vector<PrePrepareMsg> &O)
  : next_view(nv), view_changes(V), pre_prepares(O) {
    serialized << next_view;

    serialized << (uint32_t)view_changes.size();
    for(auto& vc : view_changes) serialized << vc;

    serialized << (uint32_t)pre_prepares.size();
    for(auto& pp : pre_prepares) serialized << pp;
  }
...
struct HandshakeMsg {
  ...
  HandshakeMsg(uint32_t _id) : id(_id) {
    serialized << id;
  }

  HandshakeMsg(DataStream &&s) {
    s >> id;
  }
};
