...
public:
  ...
  void start() {
    ...
    for (uint32_t i = 0; i < n_; i++) {
      ...
      std::cout << "[Replica " << i << "] Started on port " << (10000 + i)
                << std::endl;
    }
    ...
    client_net_->reg_handler(
        salticidae::generic_bind(&TestCluster::on_reply, this, _1, _2));
  ...
  void send_request(const std::string &operation) {
    ...
    std::cout << "[Client] Sending: " << operation << " to primary cluster"
              << std::endl;
  ...
  void broadcast_request(const std::string &operation) {
    RequestMsg req(operation, request_counter_++, client_id_);
    std::cout << "[Client] Sending broadcast: " << operation << " to cluster"
              << std::endl;
    for (uint32_t i = 0; i < n_; i++) {
  ...
  void set_byzantine_behavior(uint32_t id, ExecutionHook hook) {
    ...
    std::cout << "[Cluster] Injected byzantine behavior into replica " << id
              << std::endl;
  }
  ...
  // Wait untill a number of executions have been observed in the service for
  // each node
  bool wait_for_operations(size_t expected_ops, uint32_t n_reps,
    ...
    while (true) {
      ...
      if (elapsed > timeout_ms) {
        std::cout << "[Wait] Timeout waiting for " << expected_ops
                  << " operations" << std::endl;
        return false;
      }
  ...
  bool wait_for_viewchange(uint32_t expected_view,
    ...
    while (true) {
      ...
      if (elapsed > timeout_ms) {
        std::cout << "[Wait] Timeout waiting for view " << expected_view
                  << std::endl;

        // Show current state
        for (const auto &rep : replicas) {
          if (!rep.crashed) {
            std::cout << "  Replica " << rep.id << ": view=" << rep.node->view_
                      << std::endl;
          } else {
  ...
  bool verify_consensus() {
    ...
    if (consensus) {
      std::cout << "[Verify] CONSENSUS: Enough correct replicas agree ("
                << agreed_counter << ", " << agreed_ops << ")" << std::endl;
    } else {
  ...
  bool verify_current_view(uint32_t view) {
    for (const auto &rep : replicas) {
      if (!rep.crashed && rep.node->view_ != view) {
        std::cout << "[Verify] VIEW: System is not in view: " << view
                  << std::endl;
        for (const auto &rep : replicas) {
          if (!rep.crashed) {
            std::cout << "  Replica " << rep.id << ": view=" << rep.node->view_
...
bool test_byzantine() {
  ...
  cluster.set_byzantine_behavior(
      byzanite_node, [](const std::string &original_op) -> std::string {
  ...
  cluster.forge_broadcast(
      byzanite_node, PrepareMsg(view, seq_num, fake_digest, byzanite_node));
...
bool test_checkpoint() {
  std::cout << "╔══════════════════════════════════════════════╗" << std::endl;
  std::cout << "║ TEST: Checkpointing (Garbage Collection)     ║" << std::endl;
  std::cout << "╚══════════════════════════════════════════════╝" << std::endl;
  ...
  uint32_t K = 10;
  uint32_t L = 20;
  cluster.set_config(L, K); // Set K to 10
  cluster.start();

  // We need to send enough requests to activate checkpointing
  ...
  cluster.wait_for_operations(L, 4);

  for (uint32_t i = 0; i < total_ops - L; i++) {
    cluster.send_request("INC");
  }

  // Wait for all operations to execute
  if (!cluster.wait_for_operations(total_ops, 4)) {
    passed = false;
  }

  // Verify Watermarks have moved
  ...
  for (const auto &rep : cluster.replicas) {
    ...
    // Watermarks should have advanced twice
    if (low_watermark == K * 2) {
      stable_replicas++;
    }
    // Verify Log Truncation
    if (log_size > total_ops - K * 2) {
      passed = false;
    }
  }

  if (stable_replicas != 4) {
    passed = false;
  }
...
int main() {
  ...
  std::vector<std::pair<std::string, bool (*)()>> tests = {
      ...
      {"Activate Checkpointing", test_checkpoint},
  };
